...
int CPUPaddleTick(int state) {
	...
	unsigned char checkMode = ~PINA & 0x08;
	unsigned short x = ADC;
	unsigned playerTwo = (char)x;
	...
	if(checkMode) {
        ...
        switch(state) {
			...
			if(gameMode) {
				if((playerTwo > 65) && (CPUPaddlePos < 2)) {
					state = CPUPADDLE_up;
				}
				else if((playerTwo < 65) &&(CPUPaddlePos > 0)) {
					state = CPUPADDLE_down;
				}
				else {
					state = CPUPADDLE_wait;
...
//Display Tick =============================================================
enum DISPLAY {DISPLAY_START, DISPLAY_lPaddle, DISPLAY_CPUPaddle, DISPLAY_ball, DISPLAY_score};
